...
The app is called Bar Inventory. It keeps track of what a bar has in stock and lets someone call out their counts out loud instead of stopping to write them down; the app matches what it hears against the inventory list and records the counts in a database. Most of what gets said is a simple item and a number, so I built the app to handle those cases on its own and only pass the unclear ones to the AI model. That made it better at reading counts correctly while cutting the AI API calls it needed by roughly 80 percent. It is written in TypeScript, React, and Python, with a SQL database, and versioned in Git.
